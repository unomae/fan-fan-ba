...
> 這份是「**人要做**」的手動驗收，與 `npm test`（<!-- ffb:tests -->366<!-- /ffb:tests --> 個自動化單元測試，2026-09-21）互補。
...
| **3** | 〈2026-07-28 三缺陷回歸〉4 ＋〈半接線 6 條〉15 | 19 | 90 分 | 🔶 **14/19**（08-13 automation-driven）| 剩 5 格＝Obsidian 落檔 2、摘要 3（需 API key／外部 App）|
| **4** | §1–§7 legacy 回歸 | 31 | 90 分 | 🔶 **16/31**（08-13 automation-driven）| 剩 15 項多數需 API key 或外部 App／帳號 |
...
- [x] `TC-F3-002` 通過（歷史回看＋SRS）；`TC-F3-004` **5/5 通過**——初驗 4/5，CSV 公式注入那條修完複驗才全過
      （詳見下方〈2026-08-13 TC-F3-004 執行結果〉）
...
**✅ 該缺陷已於同日修復並複驗**（KAKA 裁「修」）：`escapeVocabularyCsvCell` 對 `=` `+` `-` `@`
（含前導 tab／CR）開頭的值補 `'` 前綴，**順序是先補前綴再引號包裹**，含逗號的惡意值才會變成 `"'=..."`。
- 單元測試：新增 `prefixes formula-leading CSV cells…`，全套 **27 suites／305 tests**（304→305）passed、0 failed、0 skipped
- **反向驗**：把那行前綴拿掉 → 該案精準變紅（`1 failed, 9 passed`），還原後全綠 → 測試確實鎖得住
- **瀏覽器複驗（重打包 `dist/pkg` 後）**：同一條 fixture 的剪貼簿輸出變成 `'=cmd|' /C calc'!A0,...`，
  TC-F3-004 五項子檢查 **5/5 PASS**

以下為修復前的原始記錄，保留當證據：

**🔴 已重現的缺陷（修復前）：「複製今日 CSV」沒有公式注入防護**
...
- [x] 開含**跨來源 iframe** 的長文頁（嵌 YouTube / 廣告 / Disqus 的新聞）：浮球**只在主頁面一顆**，iframe 內不長球
      → 08-13 實測：主頁 1 顆、同源＋跨來源兩個子框架各 0 顆
- [ ] 在 **iframe 內選取文字** → 工具列仍跳出、能翻譯（frame-split 不該弄壞這條）
      → **工具列已驗**（跨來源 iframe 內確實跳出）；**「能翻譯」需 API key，未涵蓋**故不勾
- [ ] 主頁選取文字 → 工具列 / 結果卡正常
      → **工具列已驗**（3 顆按鈕、可見）；譯文結果需 API key，未涵蓋
- [x] 敏感頁（accounts.google.com 登入頁）→ 整支不啟用、無浮球
      → 08-13 實測（只載入頁面、未登入）：HTTP 200，浮球／工具列／結果卡皆 0

## 2. v1.9.6 — 訊息硬化（Tier 4）
- [ ] 設定 Obsidian Vault → 結果卡存入 Obsidian → 正常開啟、存入後切回原分頁（只允許 `obsidian://`）
- [x] 浮球「設定」→ 正常開設定頁
      → 08-13 實測：開出 `chrome-extension://<id>/options.html`、分頁鈕存在。
      **注意 `openOptionsPage()` 會聚焦既有的設定分頁而不是開新的**，驗這條前要先關掉已開的設定頁

## 3. v1.9.6 — web_accessible_resources（`use_dynamic_url`）（Tier 4）
- [x] 結果卡 / 浮球品牌字型（jf-openhuninn）正常載入、浮球 icon 正常（沒變系統預設醜字）
      → 08-13 實測：`jf-openhuninn` FontFace 狀態 `loaded`、`fonts.check`=true、字型請求 200、結果卡
      `font-family` 為 jf-openhuninn、icon 元素存在。**驗錯地方會白忙**：品牌字型只套 `#gemini-result-card *`，
      `#fanfanba-floating *` 是刻意用系統字型；而且字型「用到才載入」，沒開結果卡就量不到

## 4. v1.9.7 / Phase C — 單段翻譯（Tier 4）
- [x] 滑過一般段落時不會出現浮動翻譯鈕，也不會遮住原文
      → 08-13 實測：hover 後工具列不可見、單段鈕 0 個、段落位置未位移、無其他元素覆蓋
      （比對時要排除 `#fanfanba-floating`：浮球是常駐 UI，與整寬段落必然幾何重疊，不算「遮住」）
...
- [ ] **局部重試**（Phase C）：段落翻譯失敗時出現「重試此段」→ 點擊可重翻、失敗計數正確扣回
      → **前半已驗**：無 key 時四段全失敗，「重試此段」4 顆確實出現、可點；
      **後半「失敗計數正確扣回」未證實**（沒有 key 重試必然再失敗，終態同為「已完成 0/4 · 失敗 4」，
      看不出中間有沒有先扣回），故不勾
...
- [ ] 做幾次翻譯 / 解釋 / 優化 / 整頁翻譯 → 設定頁「隱私與功能說明」分頁 →「本機診斷摘要」數字有累加，並顯示版本、模型 API Key、高亮設定、使用紀錄與隱私邊界自檢
      → 只旁證到「失敗」計數會累加、版本與 API Key 兩行有顯示；**成功計數要有 key 才驗得到**，
      高亮設定與隱私邊界兩行未逐條核對，故不勾
- [x] 刻意清空目前模型 API Key → 自檢表顯示需要處理；補回 API Key 後顯示正常
      → 08-13 實測：無 key「缺 API Key」→ 補 dummy key（`gsk_` 形狀假值）「API Key 已設定」→ 移除後又回「缺 API Key」。
      **⚠️ 兩個眉角**：①dummy 值不合 `gsk_` 前綴會被格式驗證擋下、根本沒存進去
      ②~~**UI 清不掉目前模型的 key**——清空欄位按儲存會被必填驗證擋（畫面回「使用 Groq 模型請輸入 Groq API Key」），
      金鑰仍留著；本案的「移除」是直接刪 storage 完成的。依 `options.js` 驗證只檢查「目前選用模型」那顆 key，
      理論上先把模型切到別家 provider 就能清空該顆——**此路徑未實測**~~
      → **✅ 2026-08-25 已修復**：清空欄位＋儲存改跳 `confirm`，確認後即透過 UI 真正移除金鑰
      （`setSecrets` 對空字串即覆蓋刪除），取消則維持必填錯誤；非空但前綴錯誤仍被格式驗證擋下。
      單元測試：`tests/options.test.js › API Key 移除（清空欄位＋確認）` 3 條
      （27 suites／317 tests 全綠；反向驗——把 confirm 路徑突變成恆 false → 精準 1 failed）。
- [x] 按「清除診斷資料」→ 歸零
      → 08-13 實測：清除前「3 項需要處理…失敗 6」→ 清除後「已清除診斷資料」、使用紀錄歸零
- [x] DevTools Network：操作時**只有對 AI API 的請求**，無其他上傳（確認真的無 telemetry）
      → 08-13 實測（選字、開浮球、開單字本一輪）：非 fixture 的請求只有擴充自身資源 4 筆
      （含 `use_dynamic_url` 的 UUID host），**其他外連 0 筆**。
      **⚠️ 本機會有假訊號**：Kaspersky 對每個網頁注入 `gc.kis.v2.scr.kaspersky-labs.com`（本輪 12 筆），
      那是防毒不是擴充，人工看 DevTools 時別誤判。**本輪未含 AI API 請求，因為沒有 key**
...
- [x] 浮球 → 單字本，預設顯示「今日複習」；到期單字排序合理
      → 08-13 實測：預設 `filter=review`，到期兩筆依到期先後列出，已記得的不在清單
- [x] 在「今日複習」點「記得」→ 狀態變已記得，下一次複習約 7 天後
      → 08-13 實測：`en:due2` → `status=known`、`nextReviewAt` 距今 **7 天**
- [x] 在「今日複習」點「還不熟」→ 狀態維持還不熟，下一次複習約 1 天後
      → 08-13 實測：`en:due1` → `status=learning`、`nextReviewAt` 距今 **1 天**
      （這兩條的按鈕是 `[data-vocab-review][data-review-status]`，**不是**其他分頁用的 `[data-vocab-status]`）
- [x] 切到「錯題回看」→ 只看到還不熟 / learning 單字；點「記得」後該單字離開錯題回看清單
...
- [x] **CSV 檔案結構**（2026-09-13 由人工改為自動）：BOM `ef bb bf`、CRLF 行尾、表頭 14 欄，
      且以**獨立寫的 RFC4180 解析器 round-trip**確認含逗號／雙引號／換行的值不會讓欄位錯位，
      公式起頭的值讀回來仍帶著保護用的 `'`
      → 證據：`tests/vocabulary-backup.test.js`〈CSV round-trip〉與〈兩份實作不得漂移〉；
      `qa-reports/scripts/run-phase1-2.js` 的 TC-B4-002（改驗 BOM／CRLF／欄數）
- [ ] **Excel／Sheets 實際渲染：無自動化證據，刻意不勾**。上一格驗的是「檔案內容對不對」，
      **證明不了**「Excel 顯示成什麼樣」——公式格是否真被當純文字、中文是否真的不亂碼，
      只能人眼開檔確認。**不得用上一格代替這格**（自動驗過 ≠ 人眼驗過）
- [ ] 移除擴充功能再重裝 → 設定頁「匯入」剛才的 JSON → 單字救回、計數正確
      → **只用「清空 `storage.local` 」模擬過**（3 筆 → 0 筆 → 匯入回 3 筆，狀態列「新增 3、更新 0，共 3」）；
      **真正的移除重裝未做**（重裝會換擴充 ID／權限重授），故不勾
- [x] **匯入硬化**（review 🔴1/🟡2）：匯入一個內含 `"__proto__"` 當 key 的惡意 JSON → 不崩潰、該筆被忽略、其他單字正常匯入
      → 08-13 實測：原型未被污染、`goodword` 正常匯入、`polluted` 沒進單字本
- [x] 匯入超過 10MB 的檔 → 顯示「檔案太大」而非卡死
      → 08-13 實測：11.0MB 檔 → 「檔案太大（上限 10MB），請確認是翻翻吧的單字本備份。」
...
- [x] 釘住結果卡、最近查詢、單字本面板、單字高亮
      → 08-13 實測：釘住後 class 變 `g-show g-pinned`；最近查詢 1 筆可回開；單字本 1 列；高亮 mark 1 個
- [ ] 設定儲存 / 匯出 / 匯入、Cloud Sync 登入同步（含上面新增的 hover 設定有被一起備份 / 還原）
      → **設定匯出／匯入已驗**（匯出 4 個鍵的 JSON、重新匯入有回饋）；
      **Cloud Sync 登入同步需 Google 帳號，未涵蓋**；hover 設定是否隨檔備份未逐欄核對，故不勾

---

### 2026-08-13 Tier 4 執行結果：**16/31 打勾**（20 個自動化子案：18 PASS／0 FAIL／2 PARTIAL）

證據等級同前：automation-driven（Playwright 驅動 p2 已載入的擴充），**不是人眼手動 QA**；p2 無 API key／無 Obsidian vault。
逐項證據已就地寫在上面各條的 `→` 註記，這裡只記三件跨項目的事：

1. **本機的 Kaspersky 會對每個網頁注入 `gc.kis.v2.scr.kaspersky-labs.com`**（§5 那條 Network 檢查會看到 12 筆）。
   人工用 DevTools 驗「無 telemetry」時會撞到同一批請求，**那是防毒不是擴充**。
2. **驗 API Key 相關項目的兩個陷阱**：dummy 值不合 `gsk_` 前綴會被格式驗證擋掉；
   而清空欄位按儲存也會被必填驗證擋掉（key 不會被清除）。兩者都會讓人誤判成「自檢表壞了」。
3. **這輪自動化自己踩了 5 個假紅／假綠**，都是 harness 問題不是產品問題，記著別重犯：
   ①`/loaded/` 會 match `unloaded` → 字型狀態要精確比對 ②品牌字型只套結果卡、探浮球會量到系統字型
   ③`waitForEvent('page')` 回來時分頁還是空白，太早讀 `url()` 會拿到空字串 ④review 分頁的狀態鈕
   是 `[data-review-status]`，不是 `[data-vocab-status]` ⑤浮球容器與整寬段落必然幾何重疊，
   拿「有無重疊」判「有沒有遮住原文」要先排除它。

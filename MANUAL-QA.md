...
> 這份是「**人要做**」的手動驗收，與 `npm test`（190 個自動化單元測試）互補。
> 自動測試涵蓋純函式邏輯；以下這些只有在真實瀏覽器載入擴充功能才驗得了。
> 涵蓋版本：v1.9.6（注入面收斂）→ v1.9.8（單段翻譯 / 本機診斷）+ Phase A–C review 修正。
> 最後更新：2026-06-20。
...
- [ ] 滑過一般段落 → 右上角出現「譯」浮鈕 → 點擊 → 該段下方出現譯文卡
- [ ] 游標放在某段（或選取該段）→ 按 **Alt+T** → 翻譯該段
- [ ] 對**已翻譯過**的段落再觸發 → 應「定位」而非重複請求
- [ ] **context 調優驗證**：翻譯長文中間某段，譯文用詞符合前後文語境（不是孤立直譯）
- [ ] **hover 鈕開關**（Phase C）：設定頁「語言」分頁 →「段落 hover 翻譯鈕」設「關閉」→ 存檔 → 回頁面滑過段落**不再彈鈕**，但 **Alt+T 仍可翻**；改「開啟」→ 浮鈕回來
- [ ] **局部重試**（Phase C）：段落翻譯失敗時出現「重試此段」→ 點擊可重翻、失敗計數正確扣回
- [ ] 浮鈕不擋捲軸、滑開會消失、暫停網站（浮球 ×）後浮鈕不出現
- [ ] 與整頁翻譯混用：雙語 / 譯文 / 原文模式切換、複製譯文 / 雙語、定位原文都正常

## 5. v1.9.8 — 本機診斷
- [ ] 做幾次翻譯 / 解釋 / 優化 / 整頁翻譯 → 設定頁「隱私與功能說明」分頁 →「本機診斷摘要」數字有累加
- [ ] 按「清除診斷資料」→ 歸零
...
- `TESTING.md` 內容過期（寫 Vitest，實際為 Jest；數字停在 39 tests）— 待重寫

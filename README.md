...
- 設定頁可匯出 / 匯入設定檔；只有勾選「包含 API Keys」時，才會以使用者自訂密碼加密後匯出金鑰
- Google Drive 雲端同步只同步一般設定，不包含 API Key；包含 API Key 的設定備份自 v1.7.5 起使用密碼加密，Edge / Chromium OAuth fallback 自 v1.7.2 起支援
- 選取的文字、頁面標題與必要的鄰近文字上下文**僅傳送給使用者選擇的 AI 服務商**，用於處理當次請求；不傳送完整 URL、網站網域或瀏覽歷史
